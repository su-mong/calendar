...
public class Main {
    public static void main(String[] args) {
        ...
        try (Connection connection = DriverManager.getConnection(Constants.dbUrl,Constants.dbUser, Constants.dbPassword)) {System.out.println("Connection established successfully");
            ...
            createT = "CREATE TABLE IF NOT EXISTS users (id SERIAL PRIMARY KEY,name VARCHAR(100) NOT NULL,user_id VARCHAR(20) UNIQUE NOT NULL,password VARCHAR(100) NOT NULL,email VARCHAR(100) UNIQUE NOT NULL,notification_channel VARCHAR(100));";
            smt = connection.createStatement();
            smt.executeUpdate(createT);

            createT = "CREATE TABLE IF NOT EXISTS event_infos (id SERIAL PRIMARY KEY,host_id SERIAL NOT NULL,CONSTRAINT fk_events_host_id FOREIGN KEY(host_id) REFERENCES users(id),event_title VARCHAR(255) NOT NULL,start_date_time TIMESTAMP WITH TIME ZONE NOT NULL,end_date_time TIMESTAMP WITH TIME ZONE NOT NULL,color VARCHAR(7) NOT NULL,description TEXT);";
            smt = connection.createStatement();
            smt.executeUpdate(createT);


            createT = "CREATE TABLE IF NOT EXISTS events (id SERIAL PRIMARY KEY,event_info_id SERIAL NOT NULL,CONSTRAINT fk_event_info_id FOREIGN KEY(event_info_id) REFERENCES event_infos(id),user_id SERIAL NOT NULL,CONSTRAINT fk_events_user_id FOREIGN KEY(user_id) REFERENCES users(id),color VARCHAR(7) NOT NULL,reminder_interval INT DEFAULT 0,  reminder_time_frame INT );";
            smt = connection.createStatement();
            smt.executeUpdate(createT);

            createT = "CREATE TABLE IF NOT EXISTS reminders (id SERIAL PRIMARY KEY,event_id SERIAL,CONSTRAINT fk_reminders_event_id FOREIGN KEY(event_id) REFERENCES events(id),remind_at TIMESTAMP WITH TIME ZONE NOT NULL,showed_reminder BOOLEAN DEFAULT FALSE);";
            smt = connection.createStatement();
            smt.executeUpdate(createT);
